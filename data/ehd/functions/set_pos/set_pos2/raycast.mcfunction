...
## Set pos
execute as @s[tag=!stop_raycast] unless block ~ ~ ~ air align xyz positioned ~.5 ~.5 ~.5 as @e[type=marker,tag=vertexHolder,tag=pos2,tag=editing,limit=1] run function ehd:set_pos/set_pos2/end
...
execute as @s[tag=!stop_raycast] unless entity @e[type=marker,tag=ehd_miniature,distance=..1.5] if score .ray ehd_raycast matches 1.. if block ~ ~ ~ air positioned ^ ^ ^.1 run function ehd:set_pos/set_pos2/raycast
